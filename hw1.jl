...
student = (name = "Vlad Flore", kerberos_id = "")
...
# ╔═╡ 6cb1275c-f144-11ea-2567-25f28fe75db0
...
# ╔═╡ dff15dda-f145-11ea-29de-2327ed31a402
mean(demean([1,2,3]))
...
function matrix_to_vecvec(matrix)
	return [matrix[idx, :] for idx in 1:size(matrix,1) ]
end
...
function mean_colors(image)
	...
	mr = r / size(image,1) * size(image,2)
	mg = g / size(image,1) * size(image,2)
	mb = b / size(image,1) * size(image,2)
...
philip_inverted = invert.(copy(philip))
